...
export function Home() {
    ...
    useEffect(() => {
        const fetchLinksData = async () => {
            ...
            setLinks(userLinks);
            setData((value) => { return { ...value, loading: false,userLinks:userLinks } })
        }
        ...
        <>
            <Stack width={"100%"} height={"100%"} gap={2}>
                ...
                <Stack component={Paper} variant='outlined' height={"fit-content"}>
                    ...
                    >
                        {data.user?.links?.visits?.byCountry?.filter(item=>item.count>0).length > 0 ? data.user?.links?.visits?.byCountry?.slice(0,5).map((country) => {
                            return (
                ...
                <Stack component={Paper} variant='outlined'>
                    ...
                    >
                        {data.user?.links?.visits?.byDevice?.filter(item=>item.count>0).length>0?data.user?.links?.visits?.byDevice?.slice(0,5).map((item) => {
                            return (
...
const DetailView = ({dataRef})=>{
    ...
    useEffect(
        ()=>{
            setContent(
                data && dataRef==="countries"?data.user?.links?.visits?.byCountry?.map((item)=>{
                    return {
                        item: isoAlphaCode2ToCountries[item.country],
                        count: item.count
                    }
                }):dataRef==="links"?data.userLinks?.map((item)=>{
                    return {
                        item: item.name,
                        count: item.visits?.total||0
                }
                }):
                data && dataRef==="devices"?data.user?.links?.visits?.byDevice?.map((item)=>{
                    return {
                        item: item.device,
                        count: item.count
                    }
            ...
            setTitles(data && dataRef==="countries"?["top-countries", "country", "visits"]:dataRef==="links"?["my-links", "link", "visits"]:
